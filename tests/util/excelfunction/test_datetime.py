...
class TestDate(unittest.TestCase):
    ...
    def test_date_from_cell(self):
        ...
        C16 = ExcelUtil.crete_cell_string(column='C', row=16)
        C17 = ExcelUtil.crete_cell_string(column='C', row=17)
        C18 = ExcelUtil.crete_cell_string(column='C', row=18)
        actual = DATE(C16, C17, C18).to_str()
        self.assertEqual(expected, actual)

    def test_date_from_fixed_cell(self):
        expected = 'DATE($C$16, $C$17, $C$18)'
        C16 = ExcelUtil.crete_cell_string(column='C', row=16, fixed_column=True, fixed_row=True)
        C17 = ExcelUtil.crete_cell_string(column='C', row=17, fixed_column=True, fixed_row=True)
        C18 = ExcelUtil.crete_cell_string(column='C', row=18, fixed_column=True, fixed_row=True)
        actual = DATE(C16, C17, C18).to_str()
        self.assertEqual(expected, actual)
...
class TestYear(unittest.TestCase):
    ...
    def test_year_from_cell(self):
        expected = 'YEAR(C1)'
        C1 = ExcelUtil.crete_cell_string(column='C', row=1)
